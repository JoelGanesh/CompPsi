...
{
	template <class T>
	uint64_t SegmentationArray<T>::index(uint64_t n) const
	{
	...
	std::vector<T> SegmentationArray<T>::Log() const
	{
		...
		std::function<float_dec_100(uint64_t)> F =
			[](uint64_t n)
			{
				float_dec_100 k(n + 1);
	...
	template <class T>
	std::vector<T> SegmentationArray<T>::SegmentAbstractFunc(uint64_t N, std::function<T(uint64_t)> F) const
	{
		int k_max = index(N);
	...
	template <class T>
	SegmentationArray<T>::SegmentationArray(uint64_t N) : 
